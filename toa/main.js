...
window.addEventListener('load', ()=>{
    ...
    document.getElementById('app').innerHTML = `
        <div class="app__header">
            <h1>Timeline of Aviation</h1>
        </div>
        <div class="app__content" id="content">
            <img src="./toa/assets/person/person.png" alt="person" id="person">
    ...
    INFORMATIONS.forEach((el,i) => {
        ...
        <div style="top: ${temp}px;" class="timeline-dot"></div>

        <div style="top: calc(${temp}px - 50px); ${i % 2 == 0 ? 'left' : 'right'}: calc(53% + 20px);" class="frame" id="content-${temp}">
            <img src="./toa/assets/photo${i + 1}.jpg" alt="img">
...
window.addEventListener('scroll', () => {
    ...
    for(let i = 50; i < 4850; i += 150)
    {
        if(scrolledPixels > i - 50){
        animation(i)
        }

        if(scrolledPixels > 4720)
        {
            document.getElementById('person').style.position = 'absolute';
            document.getElementById('person').style.top = '4720px';
        }
        else{
            document.getElementById('person').style.position = 'fixed';
            document.getElementById('person').style.top = '300px';
        }
    }
